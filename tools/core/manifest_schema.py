...
def validate(manifest):
    ...
    for key in ('id', 'name', 'version', 'release'):
        if not isinstance(template.get(key), str) or not template[key]:
            raise ValueError('template.' + key + ' is required')
    if template['release'] != 'v' + template['version']:
        raise ValueError('template.release/version mismatch')
    ...
    for key in ('schema_version', 'data_format_version'):
        if not isinstance(compatibility.get(key), str) or not compatibility[key]:
            raise ValueError('compatibility.' + key + ' is required')
    files = manifest['files']
    if not isinstance(files, dict) or files.get('template_base') != 'TEMPLATE_BASE.md':
        raise ValueError('files.template_base must be TEMPLATE_BASE.md')
    return manifest
