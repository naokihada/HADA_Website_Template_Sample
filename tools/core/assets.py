...
PROVENANCE_ROOT = Path("AI/history/assets")
...
def write_provenance(root: Path, image_id: str, source: Path, route: str) -> None:
    if yaml is None:
        raise RuntimeError("PyYAML is required")
    directory = root / PROVENANCE_ROOT / image_id
...
def import_plan(root: Path, plan_path: Path, approve: bool) -> int:
    ...
    for item in read_plan(root, plan_path):
        ...
        write_provenance(root, image_id, source, route)
        entries[image_id] = {
            "image_id": image_id,
            "master": str(master_rel).replace("\\", "/"),
            "web": str(web_rel).replace("\\", "/"),
            "source": "chatgpt_image" if route == "generated" else "manual",
    ...
    print(f"IMPORTED {len(read_plan(root, plan_path))} asset(s)")
    return 0
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument("command", choices=("scan-inbox", "import", "sync", "validate"))
    ...
    try:
        ...
        if args.command == "import":
            return import_plan(root, root / safe_relative(root, args.plan_file), args.approve)
